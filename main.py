...
def solicitar_parametros():
    ...
    # 2. muni_id
    print("\n[2/6] Identificación del municipio")
    ...
    print("\n[4/6] Simulación EPANET")
    disponibles = get_result_ids_disponibles(sector_ids)
    if disponibles:
        print("  Simulaciones disponibles en la base de datos:")
        for r in disponibles:
            print(f"    - {r['result_id']}  ({r.get('tstep_count', '?')} timesteps)")
    ...
    # 6. Texto de descripción de sectores
    print("\n[6/6] Descripción de la sectorización")
...
def main():
    ...
    from config import OUTPUT_DIR
    nombre_salida = f"A006_{params['municipio_nombre'].upper().replace(' ', '_')}.docx"
